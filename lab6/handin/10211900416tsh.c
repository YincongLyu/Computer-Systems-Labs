...
void eval(char *cmdline) 
{
    ...
    if(!builtin_cmd(argv)) {//step 3 判断是否为内置命令
	
		//如果不是内置命令，阻塞SIGCHLD,防止子进程在父进程之间结束
		//也就是addjob和deletejob之间，必须保证这个前后顺序
        Sigprocmask(SIG_BLOCK, &mask_one, &prev_one);
        if((pid = Fork()) == 0) {	 //创建子进程
			
            Sigprocmask(SIG_SETMASK, &prev_one, NULL);//子进程，先解除对SIGCHLD阻塞
		...
		//step 4 创建完成子进程后，父进程addjob,整个函数执行期间，必须保证不能被中断。
		//因此，这时要阻塞所有信号
		Sigprocmask(SIG_BLOCK, &mask_all, NULL); //添加工作前阻塞所有信号
		addjob(jobs, pid, state, cmdline); //添加至作业列表
		Sigprocmask(SIG_SETMASK, &mask_one, NULL);//再次阻塞
...
int builtin_cmd(char **argv) 
{
	...
	Sigfillset(&mask_all);
    if (!strcmp(argv[0], "quit"))exit(0);//直接退出
