...
void eval(char *cmdline) 
{
    ...
    if (!builtin_cmd(argv)) { // 判断是否为内置命令
        Sigprocmask(SIG_BLOCK, &mask_one, &prev); // 避免子进程停止运行
        if ((pid = fork()) == 0) { // 子进程
            Sigprocmask(SIG_SETMASK, &prev, NULL); // 解除子进程的阻塞
            Setpgid(0, 0); // 创建一个进程组
            Execve(argv[0], argv, environ); // 执行这个进程
            exit(0);
        }
        ...
        if (state == FG) { // 前台
            ...
            addjob(jobs, pid, state, cmdline); // 添加至作业列表
            Sigprocmask(SIG_SETMASK, &mask_one, NULL);
            printf("[%d] (%d) %s",pid2jid(pid), pid, cmdline); // 打印后台进程的信息
        }
...
int builtin_cmd(char **argv) 
{
    if (!strcmp(argv[0], "quit")) { // 退出命令
    	exit(0);
    } else if (!strcmp(argv[0], "jobs")) { // 打印状态
    	listjobs(jobs);
    	return 1;
    } else if (!strcmp(argv[0], "bg") || !strcmp(argv[0], "fg")) { // 判断前后台并执行相应操作
    	do_bgfg(argv);
    	return 1;
...
void do_bgfg(char **argv) 
{
    ...
    } else if(!isdigit(argv[1][0])) {  // 既不是'%'，也不是数字
    ...
    Kill(-(job->pid), SIGCONT); // 重启进程, 发送到进程组
    job->state = state;
    if (state == BG) {
