...
void eval(char *cmdline) 
{
    ...
    if (!builtin_cmd(argv)) /* If it is not built-in, create a child process and execute it */
    {
        Sigfillset(&mask_all); /* Add every signal number to set mask_all */
        Sigemptyset(&mask_one); /* Create empty set mask_one */
        Sigaddset(&mask_one, SIGCHLD); /* Add signal number to set mask_one */
        ...
        if ((pid = Fork()) == 0) /* Child runs user job */
        {
            ...
            Setpgid(0,0); /* Set child's group to a new process group which is identical to the child's PID */
...
void do_bgfg(char **argv) 
{
    /* bg can turn the terminated background jobs into running state */
    /* fg can turn the background jobs into running in foreground state */
    if (!argv[1])
    {
...
void sigchld_handler(int sig) 
{
    ...
    int status;
    Sigfillset(&mask_all);
    /* WNOHANG | WUNTRACED makes sure it returns immediately */
    while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0)
    {
        /* Use WIF functions to catch stopped child jobs */
        ...
        else /* Stopped, only change the state */
        {
            struct job_t *job = getjobpid(jobs, pid);
            Sigprocmask(SIG_BLOCK, &mask_all, &prev_all);
