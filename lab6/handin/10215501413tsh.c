...
void eval(char *cmdline) {
    ...
    if (argv[0] == NULL) return; // no command
...
void do_bgfg(char **argv) {
    ...
    struct job_t *job;
    if(argv[1][0] == '%'){
        if((pid = strtol(&argv[1][1], NULL, 10)) <= 0){
...
void sigchld_handler(int sig) {
    ...
    while ((pid = waitpid(-1, &state, WNOHANG | WUNTRACED)) > 0) {
        struct job_t* job = getjobpid(jobs, pid);
        if (WIFSIGNALED(state) && WTERMSIG(state) == SIGINT) {
            printf("Job [%d] (%d) terminated by signal 2\n", job -> jid, job -> pid);
        } else if (WIFSTOPPED(state) && WSTOPSIG(state) == SIGTSTP) {
