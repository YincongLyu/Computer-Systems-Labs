...
void do_bgfg(char **argv) 
{
    ...
    myjob = argv[1][0] == '%' ? getjobjid(jobs, jobid) : getjobpid(jobs, jobid);
    if (!myjob) {
    ...
    myjob->state = !strcmp(argv[0], "bg") ? BG : FG;
    printf("[%d] (%d) %s", myjob->jid, myjob->pid, myjob->cmdline);
    if (myjob->state == FG) waitfg(myjob->pid);
}
