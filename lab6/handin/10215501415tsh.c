...
void eval(char *cmdline) 
{
    char* argv[MAXARGS] = {NULL};  // 存放命令行分割好的待解析的参数
    ...
    // 信号标志位上为1：阻塞
    // 如果不是内置命令，创建新的子进程，在子进程中执行
    if(!builtin_cmd(argv))
    {
        sigset_t mask_all, mask_one, prev_one;  // 存放过程中的信号
        sigfillset(&mask_all);  // 将所有标志位设为1，屏蔽所有信号
        sigemptyset(&mask_one); // 将所有标志位设为0，清空所有信号
        sigemptyset(&prev_one);
        sigaddset(&mask_one, SIGCHLD); // 将SIGCHLD（默认忽略）添加到mask_one中
        
        pid_t fpid;
        // 防止子进程被其他信号先调度，将SIGCHLD屏蔽，原来的信号存在prev_one里
        sigprocmask(SIG_BLOCK,&mask_one,&prev_one);
        // fork一个子进程，父进程中返回子进程id，子进程返回0，错误返回负数
        fpid = fork();
        ...
        {
            setpgid(0, 0);
            sigprocmask(SIG_SETMASK, &prev_one, NULL);  // 将子进程信号设置成原来的信号
            // 执行不成功
...
void do_bgfg(char **argv) 
{
    int bg_fg = (!strcmp(argv[0],"bg")) + 1;  // 存储是前台or后台状态
    
    bool argv1_read = (argv[1] != NULL);  // 判断是否有输入参数
    // 参数为空
    if(!argv1_read){
    ...
    job_ptr -> state = bg_fg;  // 重新根据指令设置指定job的状态
    kill(-job_ptr -> pid, SIGCONT);  // 发送状态，重启过程
    if(bg_fg == BG){
        printf("[%d] (%d) %s", job_ptr -> jid, job_ptr -> pid, job_ptr -> cmdline);
    }
    if(bg_fg == FG){
        waitfg(job_ptr -> pid);  // 等待前台执行完成
    }
    return;
}
...
void waitfg(pid_t pid)
{
    ...
    sigemptyset(&mask_none);  // 定义一个空信号集
    while(1){
        sigprocmask(SIG_SETMASK, &mask_none, &prev_all);
        // fgpid：返回当前前台运行进程的pid，若没有，则返回0
        int ret = fgpid(jobs);
        if(!ret){
            break;  // 没有正在运行的前台进程
        }
...
void sigchld_handler(int sig) 
{
    int olderrno = errno;  // 便于最后恢复errno
    int status;  // 存放子进程终止原因
    pid_t pid;  // 记录子进程pid(int)
    sigset_t mask_all, prev_all;
    sigemptyset(&prev_all);
    sigfillset(&mask_all);
    
    // 当任意一个子进程结束后（返回的子进程pid不为0时），从joblist中删除
    while((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0){
        sigprocmask(SIG_BLOCK, &mask_all, &prev_all);  // 屏蔽所有信号
        ...
        // 更新完状态后，此时仍不为STOP的，可以回收
        if(getjobpid(jobs, pid) -> state != ST){
            deletejob(jobs, pid);
        }
        sigprocmask(SIG_SETMASK, &prev_all, NULL);  // 恢复阻塞前信号状态
    }                     
...
void sigint_handler(int sig) 
{
    ...
    sigfillset(&mask_all);
    sigemptyset(&prev_all);
    // 获取前台运行进程pid之前，先阻塞其他信号传递后所造成的干扰
    sigprocmask(SIG_BLOCK, &mask_all, &prev_all);
    
    struct job_t* job = getjobpid(jobs, fg_pid);
    job -> state = UNDEF;
    kill(-fg_pid, 2);  // 将整个进程组信号设置为SIGINT
    printf("Job [%d] (%d) terminated by signal 2\n", fg_jid, fg_pid);
    sigprocmask(SIG_SETMASK, &prev_all, NULL);  // 恢复信号
    return;
}
